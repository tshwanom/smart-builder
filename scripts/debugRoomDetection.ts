...
interface Room { id: string, walls: string[], area: number, perimeter: number }

// Paste the CURRENT detectRooms logic here (or import if I could, but copy-paste ensures state)
// ... I will mock the helpers to avoid huge file ...

// ---------------------------------------------------------
//  ALGORITHM CORE (Pasted from source Phase 1-4 + Filters)
// ---------------------------------------------------------

function detectRooms(walls: Array<{ id: string; points: Point[] }>): Room[] {
  const EPSILON = 0.001
  const SNAP_DIST = 0.2
  ...
  const graphSegments = walls.map((w, i) => {
  ...
  const fragments: Array<{ id: string; points: Point[]; originalId: string }> = []
  ...
  const candidates = cycles.map(cycle => {
    ...
    const signedArea = calculateDebugPolygonArea(poly)
...
function calculateDebugPolygonArea(points: Point[]): number {
  let area = 0
...
function calculatePerimeter(points: Point[]) { return points.length * 0 }
interface GraphNode { id: string, point: Point, edges: string[] }
interface GraphEdge { id: string, startNode: string, endNode: string }
...
function findMinimalCycles(nodes: Record<string, GraphNode>, edges: GraphEdge[]): string[][] {
    const cycles: string[][] = []
...
// ---------------------- TEST DATA ---------------------------
// 3 Stepped Rooms
// Room 1 (Right): (10,0) - (20,0) - (20,10) - (10,10)
// Room 2 (Mid):   (5,5) - (15,5) ??? No, let's use shared walls
...
const walls = [
    ...
    // Room 2 (Mid) 10x10 square at (5,5) overlapping? No, let's say (0,10) adjacent?
    // Let's model the steps:
    // Room 1: (10,0) to (20,10)
    // Room 2: (5,5) -- NO, let's look at screenshot
    // R1 Top, R2 Below-Left, R3 Below-Left-Left...
    // Let's try 2 adjacent rects sharing a wall
    
    // Wall 1: (0,0) to (10,0)
    // Wall 2: (10,0) to (10,10) (Shared)
    // Wall 3: (10,10) to (0,10)
    // Wall 4: (0,10) to (0,0)
    
    // Room 2 to the right
    // Wall 5: (10,0) to (20,0) (Top of R2)
    // Wall 6: (20,0) to (20,10)
    // Wall 7: (20,10) to (10,10) (Bottom of R2)
    // (Left of R2 is Wall 2 shared)
    
    // So 7 walls total.
    
    // Room 1 Walls
    { id: 'r1_t', points: [{x:0,y:0}, {x:10,y:0}] },
    { id: 'shared', points: [{x:10,y:0}, {x:10,y:10}] },
    { id: 'r1_b', points: [{x:10,y:10}, {x:0,y:10}] },
    { id: 'r1_l', points: [{x:0,y:10}, {x:0,y:0}] },
    
    // Room 2 Walls
    { id: 'r2_t', points: [{x:10,y:0}, {x:20,y:0}] },
    { id: 'r2_r', points: [{x:20,y:0}, {x:20,y:10}] },
    { id: 'r2_b', points: [{x:20,y:10}, {x:10,y:10}] }
]
...
rooms.forEach((r, i) => {
    console.log(`Room ${i}: Area=${r.area.toFixed(2)}, ID=${r.id}`)
})
