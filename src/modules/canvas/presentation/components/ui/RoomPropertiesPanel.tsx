...
export const RoomPropertiesPanel: React.FC<RoomPropertiesPanelProps> = ({ room }) => {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'roof' | 'finishes'>('general')
  
  // Check if room already has a roof
  const hasRoof = roofPanels.some(panel => panel.roomId === room.id)

  return (
    <div className="bg-white rounded-lg border border-gray-200 flex flex-col h-full max-h-[80vh]">
      {/* Header */}
      <div className="p-4 border-b border-gray-200">
          ...
          <div className="flex border-b border-gray-200 mt-2">
            <button
              className={`flex-1 py-2 text-xs font-medium text-center ${activeTab === 'general' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              onClick={() => setActiveTab('general')}
            >
              General
            </button>
            <button
              className={`flex-1 py-2 text-xs font-medium text-center ${activeTab === 'roof' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              onClick={() => setActiveTab('roof')}
            >
              Roof
            </button>
            <button
              className={`flex-1 py-2 text-xs font-medium text-center ${activeTab === 'finishes' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-4">
        ...
        {activeTab === 'general' && (
            <>
                ...
                {/* Legacy Finish Selectors (Optional - can keep for now or remove since we have a tab) */}
                {/* Keeping legacy props for backward compatibility if backend relies on them distinct from FinishSchedule */}
                {/* Floor Finish */}
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                    Floor Type (General)
